...
export class CelestialSystem {
  ...
  createSun() {
    ...
    sunGlowLayers.forEach(layer => {
      ...
      const glowMaterial = new THREE.MeshBasicMaterial({ color: 0xffdd44, transparent: true, opacity: layer.opacity, side: THREE.DoubleSide, fog: false, blending: THREE.AdditiveBlending });
  ...
  createMoon() {
    ...
    const moonGlowMaterial = new THREE.MeshBasicMaterial({ color: 0xFFFFFF, transparent: true, opacity: 0.5, side: THREE.DoubleSide, fog: false, blending: THREE.AdditiveBlending });
  ...
  createStars() {
    ...
    const starsMaterial = new THREE.PointsMaterial({ vertexColors: true, size: 1.5, transparent: true, opacity: 0, fog: false, sizeAttenuation: false });
  ...
  updateStarTwinkle() {
    ...
    for (let i = 0; i < sizes.length; i++) {
      sizes[i] = this.originalStarSizes[i] * (Math.sin(this.starsTwinkleTime + i * 0.25) * 0.2 + 0.8);
    }
  ...
  update(camera, directionalLight) {
    ...
    if (this.gameHour !== this.lastStreetLightHour) {
      ...
      }).catch(error => console.warn('Could not update street lights:', error));
      this.lastStreetLightHour = this.gameHour;
    }
    
    // --- Perhitungan Siklus Siang-Malam ---
    const preciseMinutes = this.gameHour * 60 + this.gameMinute + this.minuteAccumulator;
    ...
    if (height > 0) { // Matahari di atas cakrawala
      ...
      if (sunHeightFactor < 0.2) { // Sunrise
        this.scene.background.copy(this.SUNRISE_COLOR).lerp(this.DAY_COLOR, sunHeightFactor / 0.2);
      } else if (sunHeightFactor < 0.9) { // Day
        this.scene.background.copy(this.DAY_COLOR);
      } else { // Sunset
        this.scene.background.copy(this.DAY_COLOR).lerp(this.SUNSET_COLOR, (sunHeightFactor - 0.9) / 0.1);
      }
      this.scene.fog = this.dayFog;
      ...
      this.stars.material.opacity = Math.min(1.0, this.stars.material.opacity + 0.02);
